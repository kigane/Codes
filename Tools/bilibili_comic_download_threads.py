...
class Comic:
    ...
    def fetch(self):
        ...
        if rep.ok:
            ...
            # 开始爬取
            with Progress() as progress, ThreadPoolExecutor(max_workers=8) as pool:
                # fan-out. 用类的实例方法作map的参数应该要手动绑定方法。
                for _ in pool.map(Episode.download, self.episodes, [progress] * len(self.episodes)):
                    pass
                # auto fan-in
